...
def predict_match(home_team, away_team, home_rest_days, away_rest_days, b365h=2.0, b365d=3.0, b365a=3.0, referee_name="Unknown"):
    ...
    model_path = 'laliga_rf_model.pkl'
    ...
    home_missing_assists = home_health['Missing_Assists_Pct'].values[0] if len(home_health) > 0 else 0
    home_missing_np_goals = home_health['Missing_NP_Goals_Pct'].values[0] if len(home_health) > 0 else 0
    home_missing_yellows = home_health['Missing_Yellows_Pct'].values[0] if len(home_health) > 0 else 0
    home_missing_reds = home_health['Missing_Reds_Pct'].values[0] if len(home_health) > 0 else 0
    ...
    away_missing_assists = away_health['Missing_Assists_Pct'].values[0] if len(away_health) > 0 else 0
    away_missing_np_goals = away_health['Missing_NP_Goals_Pct'].values[0] if len(away_health) > 0 else 0
    away_missing_yellows = away_health['Missing_Yellows_Pct'].values[0] if len(away_health) > 0 else 0
    ...
    missing_goals_diff = home_missing_goals - away_missing_goals
    missing_assists_diff = home_missing_assists - away_missing_assists
    missing_np_goals_diff = home_missing_np_goals - away_missing_np_goals
    missing_yellows_diff = home_missing_yellows - away_missing_yellows
    missing_reds_diff = home_missing_reds - away_missing_reds
...
if __name__ == "__main__":
    ...
    predict_match("Levante", "Osasuna", 6, 6, 2.50, 3.50, 2.70, "Jose Munuera")
    predict_match("Sevilla", "Sociedad", 8, 8, 2.40, 3.50, 2.88, "Alejandro Hernández")
